...
const gameReducer = (state: GameState, action: GameAction): GameState => {
  switch (action.type) {
    ...
    case 'SELECT_TILE':
      if (state.selectedTiles.length === 2) return state;
      if (state.selectedTiles.includes(action.payload)) return state; // Prevent double-click selection
      return { ...state, selectedTiles: [...state.selectedTiles, action.payload] };
    case 'REMOVE_TILES': {
      const matchedTiles = state.selectedTiles;
      return {
        ...state,
        tiles: state.tiles.filter(tile => !matchedTiles.includes(tile.id)),
        selectedTiles: []
      };
    }
